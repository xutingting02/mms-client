        ...
        <div class="add-sale">
            <el-table :data="tableData" border stripe style="width: 100%;">
                ...
                <el-table-column prop="number" label="数量">
                    <template slot-scope="scope">
                        <el-input-number v-if="scope.row.edit" v-model="scope.row.number" :min="1" :max="getMedicineNumber(scope.row.medicine)" label="数量"  style="width: 200px;"></el-input-number>
...
import { addSale, getSaleById } from '@/api/sale'
...
export default {
    ...
    methods: {
        ...
        getMedicineNumber(id) {
            if (!id) {
                return 0;
            }
            return this.medicines.find(item => item.id === id).number;
        }
    },
    created() {
        if (this.$route.params.id) {
            this.isEdit = true
            getSaleById({ id: this.$route.params.id }).then((res) => {
                this.form = res
            })
